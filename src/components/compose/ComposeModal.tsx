        ...
        {instance.attachments.length > 0 && (
          <div className="px-3 py-2 border-b border-slate-200 dark:border-slate-800 flex flex-wrap gap-2 bg-slate-50/50 dark:bg-slate-800/30">
            {instance.attachments.map((att) => {
              const cleanName = getCleanFileName(att);
              const sizeKb = att.sizeBytes ? `${(att.sizeBytes / 1024).toFixed(1)} KB` : '';
              return (
                <div
                  key={att.id}
                  className="flex items-center space-x-2 px-3 py-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 shadow-sm"
                >
                  <FileText className="w-3.5 h-3.5 text-mexo-600 dark:text-mexo-400 flex-shrink-0" />
                  <span className="truncate max-w-[140px]" title={cleanName}>
                    {cleanName}
                  </span>
                  {sizeKb && <span className="text-[10px] text-slate-400 font-mono">{sizeKb}</span>}
